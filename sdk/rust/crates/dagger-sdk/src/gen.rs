...
impl ModuleSource {
    ...
    /// * `path` - The path on the caller's filesystem to load.
    /// * `opt` - optional argument, see inner type for documentation, use <func>_opts to use
    pub fn resolve_directory_from_caller_opts(
        &self,
        path: impl Into<String>,
...
impl Query {
    ...
    /// * `source` - The source of the dependency
    /// * `opt` - optional argument, see inner type for documentation, use <func>_opts to use
    pub fn module_dependency_opts(
        &self,
        source: ModuleSource,
        opts: QueryModuleDependencyOpts
    ) -> ModuleDependency {
        let mut query = self.selection.select("moduleDependency");
